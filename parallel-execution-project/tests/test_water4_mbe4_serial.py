...
This script tests serial execution for the 4-water cluster calculation to validate
that our AtomicResult analysis fixes work for both parallel and serial modes.
"""
...
def run_water4_serial_calculation():
    """Run the many-body expansion calculation for 4-water cluster with serial execution."""

    # Create the 4-water cluster molecule
    ...
    # Set up serial many-body calculation using ManyBodyComputer (traditional approach)
    from qcmanybody import ManyBodyCore, BsseEnum
    from qcmanybody.computer import ManyBodyComputer

    # Create ManyBodyComputer for serial execution
    mb_input = {
        "molecule": mol,
        "specification": {
            "specification": {
    ...
    print(f"System: {len(mol.symbols)} atoms in {len(mol.fragments)} fragments")
    ...
    computer = ManyBodyComputer.from_manybodyinput(mb_input)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("AtomicResult analysis fixes work for both serial and parallel modes!")
        print("="*60)
