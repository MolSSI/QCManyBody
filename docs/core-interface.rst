...
The core interface is accessed through the :class:`~qcmanybody.core.ManyBodyCore`
class.
...
Next, create a :class:`~qcmanybody.core.ManyBodyCore` object. This object is constructed using the molecule,
...
.. note::
    The core interface is less flexible than the high-level interface when it comes to the ``levels`` option.
...
For a complete discussion of the other options available in the :class:`~qcmanybody.core.ManyBodyCore` object, see the
:doc:`keywords` and the :class:`~qcmanybody.core.ManyBodyCore` API documentation.

The next step is to obtain the calculations to be run from the :class:`~qcmanybody.core.ManyBodyCore` object.
This is done with a python generator function :func:`~qcmanybody.core.ManyBodyCore.iterate_molecules` that returns
...
See `test_example.py <https://github.com/MolSSI/QCManyBody/blob/main/qcmanybody/tests/test_examples.py>`_
...
The level is the opaque label as given from the :class:`~qcmanybody.core.ManyBodyCore` .
...
For a discussion about what the results contain, see the :doc:`results`.
