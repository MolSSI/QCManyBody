...
def assemble_nbody_components(
    ...
    if 1 in nbodies:
        ...
        if ptype == "energy":
            monomer_energy_list = [
                component_results[labeler(m)] for m in monomers_in_monomer_basis
            ]
    ...
    if "cp" in metadata["bsse_type"]:
        ...
        if ptype == "energy":
            ...
            if monomer_sum != 0.0:
                nbody_dict["CP-CORRECTED TOTAL ENERGY"] = cp_body_dict[
                    metadata["max_nbody"]
                ]
            nbody_dict["CP-CORRECTED INTERACTION ENERGY"] = (
                cp_body_dict[metadata["max_nbody"]] - cp_body_dict[1]
            )

            for nb in nbodies[1:]:
                nbody_dict[f"CP-CORRECTED INTERACTION ENERGY THROUGH {nb}-BODY"] = (
                    cp_body_dict[nb] - cp_body_dict[1]
                )
                nbody_dict[f"CP-CORRECTED {nb}-BODY CONTRIBUTION TO ENERGY"] = (
                    cp_body_dict[nb] - cp_body_dict[nb - 1]
                )
            for nb in nbodies:
                nbody_dict[f"CP-CORRECTED TOTAL ENERGY THROUGH {nb}-BODY"] = (
                    cp_body_dict[nb]
                )
    ...
    if "nocp" in metadata["bsse_type"]:
        ...
        if ptype == "energy":
            ...
            nbody_dict["NOCP-CORRECTED TOTAL ENERGY"] = nocp_body_dict[
                metadata["max_nbody"]
            ]
            nbody_dict["NOCP-CORRECTED INTERACTION ENERGY"] = (
                nocp_body_dict[metadata["max_nbody"]] - nocp_body_dict[1]
            )
    ...
    if "vmfc" in metadata["bsse_type"]:
        ...
        if ptype == "energy":
            ...
            vmfc_interaction_energy = (
                vmfc_body_dict[metadata["max_nbody"]] - vmfc_body_dict[1]
            )
...
class ManyBodyComputer(BaseComputer):
    ...
    max_nbody: int = Field(
        -1,
        description="Maximum number of bodies to include in the many-body treatment. Possible: "
        "max_nbody <= nfragments. Default: max_nbody = nfragments.",
    )

    nbodies_per_mc_level: List[List[Union[int, Literal["supersystem"]]]] = Field(
        [],
        description="Distribution of "
        "active n-body levels among model chemistry levels. All bodies in range [1, self.max_nbody] must be present "
        "exactly once. Number of items in outer list is how many different modelchems. Each inner list specifies what "
        "n-bodies to be run at the corresponding modelchem (e.g., `[[1, 2]]` has max_nbody=2 and 1-body and 2-body "
        "contributions computed at the same level of theory; `[[1], [2]]` has max_nbody=2 and 1-body and 2-body "
        "contributions computed at different levels of theory. An entry 'supersystem' means all higher order n-body "
        "effects up to the number of fragments. The n-body levels are effectively sorted in the outer list, "
        "and any 'supersystem' element is at the end.",
    )  # formerly nbody_list
    ...
    def set_embedding_charges(cls, v, values):
        if len(v) != values["nfragments"]:
            raise ValueError(
                "embedding_charges dict should have entries for each 1-indexed fragment."
            )
    ...
    def build_tasks(
        ...
        **kwargs: Dict[str, Any],
        ...
        info = (
            "\n"
            + p4util.banner(
                f" ManyBody Setup: N-Body Levels {nbodies}", strNotOutfile=True
            )
            + "\n"
        )
        core.print_out(info)
        ...
        if nbodies == ["supersystem"]:
            ...
            compute_dict = build_nbody_compute_list(
                ["nocp"],
                list(range(1, self.max_nbody + 1)),
                self.nfragments,
                self.return_total_data,
            )
        else:
            compute_dict = build_nbody_compute_list(
                self.bsse_type, nbodies, self.nfragments, self.return_total_data
            )
        ...
        for nb in compute_dict["all"]:
            for pair in compute_dict["all"][nb]:
                ...
                if self.embedding_charges:
                    embedding_frags = list(
                        set(range(1, self.nfragments + 1)) - set(pair[1])
                    )
                    charges = []
                    for frag in embedding_frags:
                        positions = (
                            self.molecule.extract_subsets(frag).geometry().np.tolist()
                        )
                        charges.extend(
                            [
                                [chg, i]
                                for i, chg in zip(
                                    positions, self.embedding_charges[frag]
                                )
                            ]
                        )
                    data["keywords"]["function_kwargs"].update(
                        {"external_potentials": charges}
                    )
    ...
    def prepare_results(
        ...
        results_list = {
            k: v.get_results(client=client)
            for k, v in (results.items() or self.task_list.items())
        }
        trove = {  # AtomicResult.properties return None if missing
            "energy": {k: v.properties.return_energy for k, v in results_list.items()},
            "gradient": {
                k: v.properties.return_gradient for k, v in results_list.items()
            },
            "hessian": {
                k: v.properties.return_hessian for k, v in results_list.items()
            },
        }
        ...
        if self.driver.name == "energy":
            nbody_results = assemble_nbody_components(
                "energy", trove["energy"], metadata.copy()
            )
            ...
            nbody_results.update(
                assemble_nbody_components(
                    "gradient", trove["gradient"], metadata.copy()
                )
            )
            ...
            nbody_results.update(
                assemble_nbody_components("hessian", trove["hessian"], metadata.copy())
            )
        ...
        for idx, task in results_list.items():
            ...
            nbody_results["intermediates"][
                f"N-BODY ({frag})@({bas}) TOTAL ENERGY"
            ] = task.properties.return_energy
    ...
    def get_results(
        self, client: Optional["qcportal.FractalClient"] = None
    ) -> AtomicResult:
