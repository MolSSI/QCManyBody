...
The molecule used by QCManyBody is a `QCElemental Molecule object <https://molssi.github.io/QCElemental/model_molecule.html>`_.
...
types can be specified, in which case the :doc:`results` will include separate fields for each type of
correction.
...
In the :doc:`core-interface`, all levels must be accounted for (that is, keys must go
